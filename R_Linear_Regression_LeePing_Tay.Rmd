...
```{r }
df_ipeds_data <- as.data.frame(read_csv('ipeds/hd2017.csv'))
```
...
```{r }
tuitionfee <- as.data.frame(read_csv("ipeds/ic2017_ay.csv"))
```
...
```{r }
adm <- as.data.frame(read_csv("ipeds/adm2017.csv"))
```
...
```{r }
cscore_all <- read.csv('collegescorecard/Most-Recent-Cohorts-All-Data-Elements.csv')
```
...
```{r }
class(df_ipeds_main$CHG2AY3)
```

 Filter data on public and private not-for-profit colleges
...
```{r }
top10_high_tuitionfee <- df_ipeds_main %>%
  mutate(CHG2AY3 = as.numeric(CHG2AY3) ) %>%
...
```{r }
top10_low_tuitionfee <- df_ipeds_main %>%
  mutate(CHG2AY3 = as.numeric(CHG2AY3) ) %>%
  filter(!is.na(CHG2AY3)) %>%
  select(INSTNM, Region, CHG2AY3) %>%
  top_n(n=10, wt = -CHG2AY3) %>% 
...
```
 ...
 correlation coefficient r = -0.2996377
...
```{r }
ggplot(df_ipeds_adm, aes(x=adm_rate, y=CHG2AY3)) +
```
